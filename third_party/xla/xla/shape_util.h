...
class ShapeUtil {
 public:
  ...
  // Calls the given pre-order and post-order visitor functions for each
  // subshape of the given shape.
  // Subshapes are visited in DFS starting with the `index`.
  //
  // The visitor function(s) must have the signature
  //
  //   void fn(const Shape& subshape, const ShapeIndex& index)
  template <typename PreFn, typename PostFn>
  static absl::Status ForEachSubshapeWithStatus(const Shape& shape,
                                                PreFn&& pre_fn,
                                                PostFn&& post_fn,
                                                ShapeIndex& index) {
    TF_RETURN_IF_ERROR(pre_fn(shape, index));
    if (shape.IsTuple()) {
      for (int64_t i = 0; i < ShapeUtil::TupleElementCount(shape); ++i) {
        index.push_back(i);
        TF_RETURN_IF_ERROR(ForEachSubshapeWithStatus(shape.tuple_shapes(i),
                                                     pre_fn, post_fn, index));
        index.pop_back();
      }
    }
    TF_RETURN_IF_ERROR(post_fn(shape, index));
    return absl::OkStatus();
  }

  // Calls the given pre-order and post-order visitor functions for each
  // subshape of the given shape.
  // Subshapes are visited in DFS starting with the entire shape (index {}).
  //
  // The visitor function(s) must have the signature
  //
  //   void fn(const Shape& subshape, const ShapeIndex& index)
  ...
  static absl::Status ForEachSubshapeWithStatus(const Shape& shape,
                                                ...
                                                PostFn&& post_fn) {
    ShapeIndex index;
    return ForEachSubshapeWithStatus(shape, pre_fn, post_fn, index);
  }
