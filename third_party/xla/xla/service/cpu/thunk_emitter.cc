  ...
  if (PotentiallyImplementedAsEigenConvolution(*instruction,
    ...
    if (LayoutUtil::IsMonotonicWithDim0Major(input_shape.layout()) &&
      ...
      return ThunkSequence::Of<ConvolutionThunk>(
          ThunkInfo(instruction), hlo_module_config_.debug_options(),
          input_buffer, input_shape, kernel_buffer, kernel_shape, output_buffer,
          output_shape, instruction->convolution_dimension_numbers(),
          instruction->window(), instruction->feature_group_count());
    }
  }
