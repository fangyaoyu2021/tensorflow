...
absl::Status IrEmitter::HandleCustomCall(HloInstruction* custom_call) {
  ...
  if (custom_call->shape().IsTuple()) {
    if (is_typed_ffi) {
      // It contains the root tuple to handle its case separately
      std::vector<std::vector<llvm::Value*>> nested_tuples(1);
      // Pre-order visitor to emit the buffer pointer or gather nested tuples
      auto pre_fn = [&](const Shape& shape, ShapeIndex& index) -> absl::Status {
        if (shape.IsTuple()) {
          nested_tuples.emplace_back();
        } else {
          TF_ASSIGN_OR_RETURN(BufferAllocation::Slice slice,
                              assignment_.GetUniqueSlice(custom_call, index));
          buffer_ptrs.push_back(EmitBufferPointer(slice, shape));
          nested_tuples.back().push_back(buffer_ptrs.back());
        }
        return absl::OkStatus();
      };
      // Post-order visitor to emit each nested tuple
      auto post_fn = [&](const Shape& shape,
                         ShapeIndex& index) -> absl::Status {
        if (!shape.IsTuple()) {
          return absl::OkStatus();
        }
        if (nested_tuples.back().empty()) {
          return Unimplemented("Empty tuple not implemented");
        }
        llvm::Type* ir_type = IrShapeType(shape);
        TF_ASSIGN_OR_RETURN(BufferAllocation::Slice slice,
                            assignment_.GetUniqueSlice(custom_call, index));
        llvm::Value* child_tuple = EmitBufferPointer(slice, shape);
        llvm_ir::EmitTuple(llvm_ir::IrArray(child_tuple, ir_type, shape),
                           nested_tuples.back(), &b_);
        nested_tuples.pop_back();
        // Push the child tuple base pointer into the parent tuple, e.g.:
        // (a, b, *, e) - parent tuple (`*` means the child tuple base pointer)
        //        |
        //       (c, d) - child tuple
        nested_tuples.back().push_back(child_tuple);
        return absl::OkStatus();
      };
      // Emit nested tuples in the output
      const auto tuple_count =
          ShapeUtil::TupleElementCount(custom_call->shape());
      ShapeIndex index(1);  // We need to add root index because we want to emit
                            // the final tuple separately for the custom call
                            // instruction
      for (int64_t i = 0; i < tuple_count; ++i) {
        ...
        TF_RETURN_IF_ERROR(ShapeUtil::ForEachSubshapeWithStatus(
            elem_shape, pre_fn, post_fn, index));
      }
      DCHECK_EQ(nested_tuples.size(), 1);
      llvm_ir::EmitTuple(GetIrArrayFor(custom_call), nested_tuples.back(), &b_);
    } else {  // API version is not typed FFI
      for (int i = 0; i < ShapeUtil::TupleElementCount(custom_call->shape());
        ...
        buffer_ptrs.push_back(EmitBufferPointer(slice, elem_shape));
      }
      llvm_ir::EmitTuple(GetIrArrayFor(custom_call), buffer_ptrs, &b_);
    }
  } else if (is_typed_ffi) {  // FFI single buffer output
    buffer_ptrs.push_back(output_address);
  }
  ...
  switch (typed_custom_call->api_version()) {
    ...
    case CustomCallApiVersion::API_VERSION_TYPED_FFI: {
      // Flatten only raw buffers to avoid (nested) tuples
      ...
      if (emit_code_for_msan_) {
        ...
        EmitCallToFunc("__msan_unpoison",
                       {results_alloca,
                        llvm::ConstantInt::get(
                            intptr_type,
                            *results_alloca->getAllocationSizeInBits(dl) / 8)},
                       b_.getVoidTy());
      }
