...
struct ConvolutionDimensions {
  ...
  int output_size =
      ...
      1;  // Correct for 0 padding, default stride, default dilation.
};
...
DebugOptions MakeDebugOptions() {
  ...
  debug_options.set_xla_cpu_multi_thread_eigen(false);
  debug_options.set_xla_cpu_use_mkl_dnn(false);
  debug_options.set_xla_cpu_use_acl(false);
...
class ConvolutionThunkBuilder {
 public:
  auto Build(int convolution_rank,
    ...
    auto debug_options = MakeDebugOptions();
    ...
    return ConvolutionThunk::Create(
        {"convolution"}, debug_options, std::move(input_slice), input_shape,
...
TEST(ConvolutionThunkTest, CreationErrorOnUnsupportedType) {
  ...
  ASSERT_FALSE(status_or_thunk.status().ok());
}
