...
TEST(BasicStringArrayLayoutTest, Equality) {
  ...
  // Two BasicStringArrayLayouts with the same number of dimensions are equal.
  BasicStringArrayLayout layout_2(5);
  const PjRtLayout& layout_3 = layout_2;
  EXPECT_EQ(layout_1, layout_3);

  // In the next two tests, EXPECT_NE is not used here because the version
  // available in open sourced libraries used by by copybara presubmit require
  // `operator!=` to be defined.

  // Two BasicStringArrayLayouts with different number of dimensions are not
  // equal.
  BasicStringArrayLayout layout_4(6);
  const PjRtLayout& layout_5 = layout_4;
  EXPECT_THAT(layout_5 == layout_1, testing::IsFalse());
  ...
  EXPECT_THAT(layout_7 == layout_1, testing::IsFalse());
}
